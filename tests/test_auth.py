...
@pytest.mark.asyncio
async def test_exempt_prefixes_public():
    s = _enabled_settings(auth_exempt_prefixes=("/image/",))
    _install_app_state(s, EHService(s))
    # any path under the prefix is public (client <img> loads can't send auth)
    resp = await _request("/image/fetch?url=https%3A//ehgt.org/w/02/597/x.webp")
    assert resp.status_code != 401
    resp = await _request("/image/123/abc/thumb")
    assert resp.status_code != 401
    # an unrelated path is still protected
    resp = await _request("/opds/v2.0")
    assert resp.status_code == 401


@pytest.mark.asyncio
async def test_exempt_prefixes_parse_from_env(monkeypatch):
    monkeypatch.setenv("AUTH_USERNAME", "reader")
    monkeypatch.setenv("AUTH_PASSWORD", "secret")
    monkeypatch.setenv("AUTH_EXEMPT_PREFIXES", "/image/ , /stream/")
    from app.config import load_settings

    s = load_settings()
    assert s.auth_enabled is True
    assert s.auth_exempt_prefixes == ("/image/", "/stream/")
