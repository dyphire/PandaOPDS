...
class EHClient:
    ...
    async def fetch_image_bytes(
        ...
        *,
        ...
        HTML error pages (200 with ``text/html`` body e.g. ``Invalid token`` /
        ``Invalid request`` / ``An error has occurred``) are detected and
        surfaced as ``EHServerError(retryable=True)`` so the service layer can
        trigger a source switch (``?nl=`` / re-resolve ``/s/``) instead of
        caching the error page as image bytes — mirroring JHenTai's
        ``EHImageExceptionMatcher -> reParse`` path.
        """
        from urllib.parse import urlsplit as _urlsplit

        # JHenTai's text/html error markers (EHImageExceptionMatcher) — these
        # are returned as 200 with an HTML body, not as HTTP errors.
        _HTML_ERROR_MARKERS = ("Invalid token", "Invalid request", "An error has occurred")

        headers: dict[str, str] = {}
        if referer:
            headers["Referer"] = referer
        host = _urlsplit(url).netloc or url
            ...
            try:
                async with self._client.stream("GET", url, headers=headers) as resp:
                    ...
                    if resp.status_code >= 400:
                        ...
                        raise EHServerError(
                            f"image host {host} returned HTTP {resp.status_code}",
                            retryable=True,
                        )
                    ctype = resp.headers.get("content-type", "") or resp.headers.get("Content-Type", "")
                    chunks = [c async for c in resp.aiter_bytes()]
                    data = b"".join(chunks)
                    # Detect HTML error pages masquerading as images (200 text/html).
                    # These must not be cached and should trigger a source switch.
                    if "text/html" in ctype.lower() or ctype == "":
                        # Small pages (< ~200KB) that claim to be HTML are almost
                        # certainly error pages — inspect the body.
                        if data and b"<" in data[:2048]:
                            snippet = data[:2048].decode(errors="ignore")
                            if any(m in snippet for m in _HTML_ERROR_MARKERS) or "E-Hentai" in snippet[:512]:
                                raise EHServerError(
                                    f"image host {host} returned HTML error page ({snippet[:180]!r})",
                                    retryable=True,
                                )
                            # Blank HTML body — also retry with new source (mirrors
                            # EHImageExceptionMatcher blankImage -> reParse).
                            if not snippet.strip():
                                raise EHServerError(
                                    f"image host {host} returned blank HTML body",
                                    retryable=True,
                                )
                    if not data:
                        raise EHServerError(
                            f"image host {host} returned blank image body",
                            retryable=True,
                        )
                    return data
            except (httpx.TimeoutException, httpx.TransportError) as exc:
                if attempt <= self.retries:
                    logger.warning(
                        "image fetch host=%s failed (%s), retry %d/%d url=%s",
                        host, type(exc).__name__, attempt, self.retries, url,
                    )
                    await asyncio.sleep(min(0.5 * attempt, 3.0))
                    continue
                raise EHException(
                    f"image fetch host={host} failed after {attempt} attempts: {exc}"
                ) from exc
            except EHException as exc:
                # retryable upstream image failures (CDN 5xx / Cloudflare / HTML error)
                if exc.retryable and attempt <= self.retries:
                    logger.warning(
                        "image fetch host=%s failed (%s: %s), retry %d/%d url=%s",
                        host, type(exc).__name__, exc, attempt, self.retries, url,
                    )
                    await asyncio.sleep(min(0.5 * attempt, 3.0))
                    continue
                raise
