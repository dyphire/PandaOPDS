...
def path_is_prefix_exempt(path: str, settings: Settings) -> bool:
    """True when a path starts with any configured ``AUTH_EXEMPT_PREFIXES``.

    Used for client-rendered image endpoints (``<img src>`` / ``url()`` in
    comment HTML) that a browser/webview loads natively and therefore cannot
    carry an ``Authorization`` header — e.g. ``/image/fetch``.
    """
    return any(path.startswith(p) for p in settings.auth_exempt_prefixes)
...
def auth_request_ok(request: Request) -> bool:
    ...
    if request.url.path in public_paths(settings):
        return True
    if path_is_prefix_exempt(request.url.path, settings):
        return True
